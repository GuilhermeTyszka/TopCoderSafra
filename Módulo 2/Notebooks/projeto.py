...
class NomeInvalido(Exception):
    def __init__(self, mensagem='O nome digitado é inválido.'):
        self.message = mensagem
        super().__init__(self.message)
...
def abrir_base() -> list:
    try:
        with open(r'Módulo 2\Notebooks\musicos.json', 'r', encoding = 'utf-8') as arquivo:
...
def salvar_base(base: list) -> None:
    try:
        with open(r'Módulo 2\Notebooks\musicos.json', 'w', encoding = 'utf-8') as arquivo:
...
def verifica_email(email: str) -> None:
    caracteres_validos = 'abcdefghijklmnopqrstuvwxyz@_.0123456789'
...
def obter_info(dados = None, cadastrar = False, busca = False, modificar = False, **kwargs) -> dict:
    ...
    while usuario_input_genero and not busca:
        usuario_input_genero = input('Digite outro gênero musical ou pressione ENTER: ').strip().lower()
    ...
    while usuario_input_inst and not busca:
        usuario_input_inst = input('Digite outro instrumento musical ou pressione ENTER: ').strip().lower()
...
def cadastra_usuario() -> None:
    ...
    confirmacao = input('Você deseja adicionar este músico na base de dados? (S/N)').lower().strip()
    while confirmacao != 's' and confirmacao != 'n':
        confirmacao = input('Desculpe, não entendi! Você deseja adicionar este músico na base de dados? (S/N)').lower().strip()
    if confirmacao == 's':
        base_musicos.append(novo_usuario)
        salvar_base(base_musicos)
        print('\nMúsico cadastrado com sucesso!\n')
    else:
        print('\nO músico não foi cadastrado!\n')
...
def buscar_musicos() -> None:
    base_musicos = abrir_base()
    print('\nInsira as informações que deseja inserir na busca ou pressione ENTER.\n')
    info = obter_info(busca = True)
    regra_busca = input('Você deseja encontrar apenas músicos que apresentem TODAS as informações inseridas? (S/N)').lower().strip()
    while regra_busca != 's' and regra_busca != 'n':
        regra_busca = input('Desculpe, não entendi! Você deseja encontrar apenas músicos que apresentem TODAS as informações inseridas? (S/N)').lower().strip()
...
def modificar_musicos() -> None:
    ...
    while i < len(base_musicos):
        ...
        return print('Músico não encontrado na base de dados.\n'
                    'Você foi redirecionado ao menu.\n')
    print(f'Digite as novas informações sobre gêneros musicais e instrumentos do músico {base_musicos[num_cadastro]["nome"].title()},\nregistrado com o email {email_input}: \n')
    base_musicos[num_cadastro] = obter_info(modificar = True, email = email_input, nome = base_musicos[num_cadastro]["nome"])
    salvar_base(base_musicos)
    print('\n Músico modificado com sucesso!\n')
...
def menu() -> None:
    ...
    while opcao != '0':
        ...
        try:
            ...
            print(f'O email {excecao.email} digitado já está cadastrado!\n'
                    'Você foi direcionado ao menu.\n')
        except SemGenero:
